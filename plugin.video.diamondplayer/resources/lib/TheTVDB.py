# vi: set noet ff=dos :
import os, io, time, urllib.request, urllib.parse, urllib.error, zipfile, requests_cache
...
class Show(dict):
	...
	def __repr__(self):
		return '<Show %r (containing %s seasons)>' % (self.data.get('seriesname', 'instance'), len(self))
	...
	def get_poster(self, language=None, default=None):
		try:
			posters = list(self['_banners']['poster'].values())
...
class Season(dict):
	...
	def __repr__(self):
		return '<Season %d instance (containing %s episodes)>' % (self.num, len(list(self.keys())))

	def __getitem__(self, episode_number):
		return dict.__getitem__(self, episode_number)

	def has_aired(self, flexible=False):
		if len(list(self.keys())) > 0 and list(self.values())[0].has_aired(flexible):
			return True
		return False

	def get_poster(self, language=None, default=None):
		try:
			posters = list(self.show['_banners']['season']['season'].values())
			posters = [poster for poster in posters if poster['season'] == str(self.num)]
			posters.sort(key=lambda k: float(k.get('rating',0)), reverse=True)
			if language:
				posters.sort(key=lambda k: k['language']!=language)
...
class Tvdb:
	def __init__(self, language='en'):
		...
		if language in languages:
			...
			config['language'] = 'en'
		config['url_search'] = 'https://thetvdb.com/api/GetSeries.php?seriesname=%%s&language=%%s' % config
		config['url_search_by_imdb'] = 'https://thetvdb.com/api/GetSeriesByRemoteID.php?imdbid=%%s&language=%%s' % config
		config['url_sid_full'] = 'https://thetvdb.com/api/%(apikey)s/series/%%s/all/%%s.zip' %  config
		config['url_sid_base'] = 'https://thetvdb.com/api/%(apikey)s/series/%%s/%%s.xml' % config
		config['url_artwork_prefix'] = 'https://thetvdb.com/banners/%%s' % config
		self.config = config
	...
	def search(self, series, year=None, language='en'):
		series = urllib.parse.quote(to_bytes(series))
