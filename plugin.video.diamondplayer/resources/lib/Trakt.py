...
def call_trakt(path, params={}, data=None, is_delete=False, with_auth=True, pagination=False, page=1):
	params = dict([(k, text.to_utf8(v)) for k, v in list(params.items()) if v])
	headers = {
	...
	if pagination == False:
		response = send_query()
		if with_auth and response.status_code == 401 and trakt_authenticate():
			response = send_query()
...
def clear_trakt():
	title = 'diamondplayer: Clear Trakt account settings'
