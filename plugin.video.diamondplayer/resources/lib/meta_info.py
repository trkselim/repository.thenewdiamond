...
def get_movie_metadata(movie, genres_dict=None):
	...
	info['name'] = '%s (%s)' % (info['title'], info['year'])
	...
	info['poster'] = 'https://image.tmdb.org/t/p/original%s' % movie['poster_path']
	info['fanart'] = 'https://image.tmdb.org/t/p/original%s' % movie['backdrop_path']
	info['mediatype'] = 'movie'
	try:
		info['genre'] = ' / '.join([x['name'] for x in movie['genres']])
	except KeyError:
		if genres_dict:
			info['genre'] = ' / '.join([genres_dict[x] for x in movie['genre_ids']])
		else:
			info['genre'] = ''
	return info
...
def get_tvshow_metadata_trakt(show, genres_dict=None):
	...
	if genres_dict:
		try:
			info['genre'] = ' / '.join([genres_dict[x] for x in show['genres']])
		except:
			pass
	if show.get('trailer'):
...
def get_tvshow_metadata_tmdb(show, genres_dict=None):
	...
	try:
		info['genre'] = ' / '.join([x['name'] for x in show['genres']])
	except KeyError:
		if genres_dict:
			try:
				info['genre'] = ' / '.join([genres_dict[x] for x in show['genre_ids']])
			except:
				info['genre'] = ''
	info['poster'] = 'https://image.tmdb.org/t/p/original%s' % show['poster_path']
	info['fanart'] = 'https://image.tmdb.org/t/p/original%s' % show['backdrop_path']
	return info
...
def get_episode_metadata_tvdb(season_metadata, episode, banners=True):
	...
	info['title'] = '%02dx%02d. %s' % (info['season'], info['episode'], info['name'])
...
def get_episode_metadata_tmdb(season_metadata, episode):
	...
	if episode['still_path']:
		info['poster'] = 'https://image.tmdb.org/t/p/original%s' % episode['still_path']
	elif season_metadata['poster']:
		info['poster'] = 'https://image.tmdb.org/t/p/original%s' % season_metadata['poster']
	else:
		info['poster'] = ''
