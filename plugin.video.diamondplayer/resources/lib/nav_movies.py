...
def trakt_movies_popular(page, raw=False):
	...
	if raw:
		...
		return list_trakt_movies([{'movie': m} for m in results], pages, page)
...
def movies_add_all_to_library(items, noscan=False):
	...
	batch_add_file.close()
	xbmc.executebuiltin('RunPlugin(plugin://plugin.video.diamondplayer/movies/batch_add_to_library)')
...
def movies_play(src, id, usedefault='True'):
	...
	if playaction == 1 and xbmc.getCondVisibility('system.hasaddon(script.diamondinfo)') and not plugin.getProperty('infodialogs.active'):
		...
		xbmc.executebuiltin('RunScript(script.diamondinfo,info=diamondinfo,%s=%s)' % (src, id))
	else:
...
def make_movie_item(movie_info):
	...
	if xbmc.getCondVisibility('system.hasaddon(script.diamondinfo)'):
		context_menu = [
			('Movie trailer', 'RunScript(script.diamondinfo,info=playtrailer,id=%s)' % id),
	...
	try:
		if traktenabled and countenabled:
			if 'trakt_id' in list(movie_info.keys()) and movie_info['trakt_id'] != '':
			...
			movie_info.update({'playcount': len([k for d in playdata for k in list(d.keys()) if k == 'watched_at'])})
	except:
		pass
	movieitem = {
