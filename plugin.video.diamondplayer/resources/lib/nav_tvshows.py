	...
	items = sorted([
		{
			...
			'fanart': plugin.get_addon_fanart()
		} for id, name in list(genres.items())], key=lambda k: k['label'])
	return items
...
def tv_play(id, season, episode, usedefault='True'):
	...
	if playaction == 1 and xbmc.getCondVisibility('system.hasaddon(script.diamondinfo)') and not plugin.getProperty('infodialogs.active'):
		from resources.lib.play_base import action_cancel
		action_cancel()
		xbmc.executebuiltin('RunScript(script.diamondinfo,info=extendedtvinfo,tvdb_id=%s)' % (id))
	elif playaction == 2 and xbmc.getCondVisibility('system.hasaddon(script.diamondinfo)') and not plugin.getProperty('infodialogs.active'):
		from resources.lib.play_base import action_cancel
		action_cancel()
		xbmc.executebuiltin('RunScript(script.diamondinfo,info=extendedepisodeinfo,tvdb_id=%s,season=%s,episode=%s)' % (id, season, episode))
	else:
		play_tvshows.play_episode(id, season, episode, usedefault)
...
def tv_tvshow(id):
	...
	if flatten == 2:
		...
		id = int(id)
		show = TVDB[id]
		if len(list(show.items())) == 1 or (len(list(show.items())) == 2 and list(show.items())[1][0] == 1):
			plugin.set_content('episodes')
			action = 'one'
		else:
...
def tv_add_to_library_parsed(id, player):
	...
	if lib_tvshows.add_tvshow_to_library(library_folder, show, player):
		plugin.setProperty('plugin.video.diamondplayer.clean_library', 'true')
...
def tv_add_all_to_library(items, noscan=False):
	...
	batch_add_file.close()
	xbmc.executebuiltin('RunPlugin(plugin://plugin.video.diamondplayer/tv/batch_add_to_library)')
...
def build_tvshow_info(tvdb_show, tmdb_show=None):
	...
	info.update(dict((k,v) for k,v in tmdb_info.items() if v))
	return info
...
def make_tvshow_item(info):
	...
	if info['poster'] == None or info['poster'] == '':
		if tmdb_id != None and tmdb_id != '':
			...
			if show['poster_path'] != None and show['poster_path'] != '':
				info['poster'] = 'https://image.tmdb.org/t/p/w500' + show['poster_path']
			if info['fanart'] == None or info['fanart'] == '':
				if show['backdrop_path'] != None and show['backdrop_path'] != '':
					info['fanart'] = 'https://image.tmdb.org/t/p/original' + show['backdrop_path']
	if info['poster'] == None or info['poster'] == '':
	...
	if xbmc.getCondVisibility('system.hasaddon(script.diamondinfo)'):
		context_menu = [
			('diamondinfo', 'RunScript(script.diamondinfo,info=extendedtvinfo,tvdb_id=%s)' % tvdb_id),
			('TV trailer', 'RunScript(script.diamondinfo,info=playtvtrailer,tvdb_id=%s)' % tvdb_id),
	...
	try:
		if traktenabled and countenabled:
			if 'trakt_id' in list(info.keys()) and info['trakt_id'] != '':
				id = info['trakt_id']
			else:
...
def list_seasons_tvdb(id, flatten):
	...
	items = []
	for (season_num, season) in list(show.items()):
		...
		if flatten == 'none':
			if xbmc.getCondVisibility('system.hasaddon(script.diamondinfo)'):
				context_menu = [
					('diamondinfo', 'RunScript(script.diamondinfo,info=seasoninfo,tvshow=%s,season=%s)' % (show_info['name'], season_num))]
			else:
				context_menu = []
			properties = {}
			try:
...
def list_episodes_tvdb(id, season_num):
	...
	items = []
	for (episode_num, episode) in list(season.items()):
...
def _lists_trakt_show_tv_list(list_items):
	...
	for list_item in list_items:
		...
		if item_type == 'show':
			...
			try:
				if traktenabled and countenabled:
					if 'trakt' in list(list_item['show']['ids'].keys()) and list_item['show']['ids']['trakt'] != '':
