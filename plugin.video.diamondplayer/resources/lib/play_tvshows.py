import re, json, urllib.request, urllib.parse, urllib.error, datetime
import xbmc
...
def play_episode(id, season, episode, usedefault):
	...
	if link:
		plugin.setProperty('plugin.video.diamondplayer.data', json.dumps(
			{
				'dbid': dbid,
				'tvdb': id,
				'season': season,
...
def get_episode_parameters(show, season, episode):
	...
	episodes = 0
	count = 0
	for i in list(show.items()):
		episodes += len(list(i[1].items()))
		if i[0] != 0 and i[0] < (season -1):
			count += len(list(i[1].items()))
	...
	parameters['seasons'] = len(list(show.items()))
	parameters['seasons_no_specials'] = len([season_num for (season_num, season) in list(show.items()) if season_num != 0])
	...
	parameters['urlname'] = urllib.parse.quote(text.to_utf8(parameters['clearname']))
	...
	parameters['urltitle'] = urllib.parse.quote(text.to_utf8(parameters['title']))
	...
	for article in articles:
		if parameters['clearname'].startswith(article):
			parameters['sortname'] = text.to_utf8(parameters['clearname']).replace(article,'')
		if parameters['title'].startswith(article):
	...
	if parameters['genres'] != None and parameters['absolute_number'] and parameters['absolute_number'] != '0' and 'animation' in parameters['genres'].lower():
		...
		for tmdb_show in tmdb_results.get('tv_results', []):
			if 'JP' in tmdb_show['origin_country']:
				is_anime = True
	if is_anime:
		parameters['name'] = '%s %s' % (parameters['showname'], parameters['absolute_number'])
	else:
		parameters['name'] = '%s S%02dE%02d' % (parameters['showname'], parameters['season'], parameters['episode'])
...
def get_tmdb_episode_parameters(show, preason, prepisode):
	...
	if preason['episodes'][0] != None and preason['episodes'][0] != '' and preason['episodes'][0] != []:
		parameters['poster'] = 'https://image.tmdb.org/t/p/w500%s' % preason['episodes'][0]['still_path']
	elif show['poster_path'] != None and show['poster_path'] != '' and show['poster_path'] != []:
		parameters['poster'] = 'https://image.tmdb.org/t/p/w500%s' % show['poster_path']
	if show['backdrop_path'] != None and show['backdrop_path'] != '' and show['backdrop_path'] != []:
		parameters['fanart'] = 'https://image.tmdb.org/t/p/original%s' % show['backdrop_path']
	else:
		parameters['fanart'] = ''
