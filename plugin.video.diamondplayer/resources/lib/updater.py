import os, io, shutil, base64, zipfile, urllib.request, urllib.error, urllib.parse, urllib.parse, contextlib
import xbmc
...
def update_players_local(zip_path):
	extract_to = xbmc.translatePath('special://profile/addon_data/plugin.video.diamondplayer/Players')
...
def flat_extract(z, extract_to, members=None):
	...
	if not os.path.exists(extract_to):
		...
		if xbmc.getInfoLabel('Window(home).Property(running)') == 'totaldiamondplayer':
			empty_folder(extract_to)
		else:
			#if plugin.yesno('diamondplayer: Update players', 'Do you want to remove existing players first?'):
			empty_folder(extract_to)
