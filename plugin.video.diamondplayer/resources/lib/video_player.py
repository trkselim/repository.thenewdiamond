...
class VideoPlayer(xbmc.Player):
	...
	def currently_playing(self):
		video_data = plugin.getProperty('plugin.video.diamondplayer.data')
		if video_data:
			try:
	...
	def get_episode_id(self, dbid, tvdb_id, season, episode):
		...
		if dbid:
			...
			episodes = [ep for ep in episodes if 'plugin.video.diamondplayer' in ep['file'] and '/%s/' % tvdb_id in ep['file'].replace('\\', '/')]
		if episodes:
	...
	def onPlayBackEnded(self):
		...
		plugin.clearProperty('plugin.video.diamondplayer.data')
