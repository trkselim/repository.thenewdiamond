...
def multi_search_people(search_str, page=1, sort=None, order=None, filters=None, cache_days=0):
    import math
    try:
        xbmc.log(str(sort + "." + order)+'sort_by===>PHIL', level=xbmc.LOGINFO)
    except:
        pass
    params = {"append_to_response": 'combined_credits',
              ...
              "page": page}
    response = get_data(url="person/%s" % (search_str),
                        params=params,
                        cache_days=cache_days)

    if response and "combined_credits" in response:
        #response['combined_credits']['cast_crew'] = response['combined_credits']['cast'] + response['combined_credits']['crew']
        cast_crew_list = {}
        cast_crew_list['cast_crew'] = []
        for i in response['combined_credits']['cast']:
            if str("'id': " + str(i['id'])) not in str(cast_crew_list['cast_crew']) and i['poster_path'] != None:
                try: i['first_air_date'] = i['release_date']
                except: i['first_air_date'] = ''
                try: i['original_name'] = i['original_title']
                except: pass
                if i['vote_count'] == 0:
                    i['vote_count'] = 1
                if i['vote_average'] == 0:
                    i['vote_average'] = 1
                cast_crew_list['cast_crew'].append(i)
        for i in response['combined_credits']['crew']:
            if str("'id': " + str(i['id'])) not in str(cast_crew_list['cast_crew']) and i['poster_path'] != None:
                try: i['first_air_date'] = i['release_date']
                except: i['first_air_date'] = ''
                try: i['original_name'] = i['original_title']
                ...
                cast_crew_list['cast_crew'].append(i)
        x = 0
        for i in cast_crew_list['cast_crew']:
            x = x + 1
        if order == 'desc':
            reverse_order = True
        if order == 'asc':
            reverse_order = False
        if sort == 'popularity': #or sort == 'revenue':
            xbmc.log(str(sort)+'sort===>PHIL', level=xbmc.LOGINFO)
            #newlist = sorted(cast_crew_list['cast_crew'], key=lambda k: math.log10(k['vote_count'])*k['vote_average']*k['popularity'], reverse=reverse_order)
            newlist = sorted(cast_crew_list['cast_crew'], key=lambda k: k['popularity'], reverse=reverse_order)
        elif sort == 'revenue':
            newlist = sorted(cast_crew_list['cast_crew'], key=lambda k: (math.log10(k['vote_count'])*k['vote_average'])+k['popularity'], reverse=reverse_order)
        elif sort == 'original_title':
            xbmc.log(str(sort)+'sort===>PHIL', level=xbmc.LOGINFO)
            newlist = sorted(cast_crew_list['cast_crew'], key=lambda k: k['original_name'], reverse=reverse_order)
        elif sort == 'vote_average':
            xbmc.log(str(sort)+'sort===>PHIL', level=xbmc.LOGINFO)
            newlist = sorted(cast_crew_list['cast_crew'], key=lambda k: k['vote_average'], reverse=reverse_order)
        elif sort == 'vote_count':
            xbmc.log(str(sort)+'sort===>PHIL', level=xbmc.LOGINFO)
            newlist = sorted(cast_crew_list['cast_crew'], key=lambda k: k['vote_count'], reverse=reverse_order)
        elif sort == 'release_date' or sort == 'first_air_date':
            xbmc.log(str(sort)+'sort===>PHIL', level=xbmc.LOGINFO)
            newlist = sorted(cast_crew_list['cast_crew'], key=lambda k: k['first_air_date'], reverse=reverse_order)

        #newlist = sorted(response['combined_credits']['cast_crew'], key=lambda k: k['popularity'], reverse=reverse_order)
        #newlist = sorted(cast_crew_list['cast_crew'], key=lambda k: k['first_air_date'], reverse=reverse_order)
        #newlist = sorted(cast_crew_list['cast_crew'], key=lambda k: k['vote_count']*k['vote_average'], reverse=reverse_order)
        #newlist = sorted(cast_crew_list['cast_crew'], key=lambda k: k['popularity'], reverse=reverse_order)
        #newlist = sorted(cast_crew_list['cast_crew'], key=lambda k: k['vote_count']*k['vote_average']*k['popularity'], reverse=reverse_order)
        #newlist = sorted(cast_crew_list['cast_crew'], key=lambda k: math.log10(k['vote_count'])*k['vote_average']*k['popularity'], reverse=reverse_order)
        itemlist = handle_multi_search(newlist)
        #xbmc.log(str(itemlist)+'===>PHIL', level=xbmc.LOGINFO)
        #itemlist = itemlist + handle_multi_search(response['combined_credits']['crew'])
        #x = 0
        #for i in response['combined_credits']['cast_crew']:
        #    x = x + 1
        itemlist.set_totals(x)
        return itemlist
...
def clearlogo_get(name=None,tmdb_id=None,type_query=None):
    import requests
    import json
    import xbmcaddon
    import xbmc
    ...
    #tmdb_api = '2cfb516815547f7a9fb865409fe94da2'
    #fanart_api = '50f0202231883f1be2f746870f87b343'
    ...
    title = name
    if not tmdb_id:
        url = 'https://api.themoviedb.org/3/search/'+str(type_query)+'?api_key='+str(tmdb_api)+'&query='+str(title)+'&language=en-US&include_image_language=en,null'
    ...
    if str(type_query) == 'tv':
        url = 'https://api.themoviedb.org/3/'+str(type_query)+'/'+str(tmdb_id)+'/external_ids?api_key=' +str(tmdb_api)
        response = requests.get(url).json()
    ...
    if str(type_query) == 'tv':
        search_id = tvdb_id
    if str(type_query) == 'movies':
        search_id = tmdb_id
    url = 'http://webservice.fanart.tv/v3/'+str(type_query)+'/'+str(search_id)+'?api_key='+str(fanart_api)
    response = requests.get(url).json()
    
    hdtvlogo = ''
    clearlogo = ''
    for i in response:
        if i == 'clearlogo' or i == 'movielogo':
            likes = -1
            for x in response[i]:
                if x['lang'] == 'en' or str(x['lang']) == '':
                    if str(x['likes']) == '':
                        x['likes'] = 0
                    if int(x['likes']) > likes:
                        clearlogo = x['url']
                        likes = int(x['likes'])
                        print(clearlogo, likes, 'clearlogo')
        if i == 'hdtvlogo' or i == 'hdmovielogo':
            likes = -1
            for x in response[i]:
                if x['lang'] == 'en' or str(x['lang']) == '':
                    if str(x['likes']) == '':
